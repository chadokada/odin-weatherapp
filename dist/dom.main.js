...
function formatTemp(temp, unit = 'imperial'){
...
function formatWindSpeed(windSpeed, unit){
  let speedUnit = '';
  if (unit == 'imperial'){
    speedUnit = 'mph';
  } else if (unit == 'metric'){
...
async function getWeatherData(city, unit='imperial'){ //rename more
  try {
...
/* harmony export */   "highlightCurrentUnit": () => (/* binding */ highlightCurrentUnit),
...
/* harmony import */ var _weather__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./weather */ "./src/modules/weather.js");
...
async function setBackgroundImage(city){
  const imgUrl = await _weather__WEBPACK_IMPORTED_MODULE_2__.getBackground(city);
...
function displayCurrentWeather(data, unit){
  ...
  currConditions.textContent = _utils__WEBPACK_IMPORTED_MODULE_0__.toTitleCase(weather);
  currImg.src = _utils__WEBPACK_IMPORTED_MODULE_0__.getWeatherImgSrc(data.current.weather[0]['main']);
...
async function loadWeather(city, unit='imperial'){
  try {
    const weatherData = await _weather__WEBPACK_IMPORTED_MODULE_2__.getWeatherData(city, unit);
    _storage__WEBPACK_IMPORTED_MODULE_1__.setCurrentCity(city);
    displayCurrentWeather(weatherData, unit);
    displayWeeklyWeather(weatherData.daily, unit);
    displayCityName(_utils__WEBPACK_IMPORTED_MODULE_0__.toTitleCase(city));
  } catch (error) {
    console.log(error);
  }
}
...
function changeUnit(event){
  ...
  const currentCity = _storage__WEBPACK_IMPORTED_MODULE_1__.getCurrentCity();

  if (target.id == 'imperial' && target.className != 'unit-selected') {
    const metric = document.querySelector('#metric');
    toggleUnitClass(metric, target);
    _storage__WEBPACK_IMPORTED_MODULE_1__.setUnit('imperial');
    ...
    _storage__WEBPACK_IMPORTED_MODULE_1__.setUnit('metric');
...
function highlightCurrentUnit(){
  const unit = _storage__WEBPACK_IMPORTED_MODULE_1__.getUnit();
  const imperial = document.querySelector('#imperial');
  const metric = document.querySelector('#metric');
  if (unit == 'imperial') {
